# This Makefile is for the WWW::CPANRatings extension to perl.
#
...
#     ABSTRACT => q[parsing CPANRatings data]
#     AUTHOR => q[Yo-An Lin <cornelius.howl {at} gmail.com>]
#     BUILD_REQUIRES => { ExtUtils::MakeMaker=>q[6.42] }
#     DISTNAME => q[WWW-CPANRatings]
#     LICENSE => q[perl]
#     NAME => q[WWW::CPANRatings]
#     NO_META => q[1]
#     PREREQ_PM => { DateTime::Format::DateParse=>q[0], URI=>q[0], HTML::TokeParser::Simple=>q[0], ExtUtils::MakeMaker=>q[6.42], JSON::XS=>q[0], List::Util=>q[0], Web::Scraper=>q[0], Text::CSV_PP=>q[0], LWP::Simple=>q[0] }
#     VERSION => q[0.01]
#     VERSION_FROM => q[lib/WWW/CPANRatings.pm]
...
INSTALLARCHLIB = $(INSTALL_BASE)/lib/perl5/darwin-multi-2level
DESTINSTALLARCHLIB = $(DESTDIR)$(INSTALLARCHLIB)
INSTALLSITEARCH = $(INSTALL_BASE)/lib/perl5/darwin-multi-2level
DESTINSTALLSITEARCH = $(DESTDIR)$(INSTALLSITEARCH)
INSTALLVENDORARCH = $(INSTALL_BASE)/lib/perl5/darwin-multi-2level
...
MAKE = make
FULLEXT = WWW/CPANRatings
BASEEXT = CPANRatings
PARENT_NAME = WWW
DLBASE = $(BASEEXT)
VERSION_FROM = lib/WWW/CPANRatings.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/WWW/CPANRatings.pm
...
DISTNAME = WWW-CPANRatings
DISTVNAME = WWW-CPANRatings-0.01
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/WWW/CPANRatings.pm $(INST_MAN3DIR)/WWW::CPANRatings.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>parsing CPANRatings data</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Yo-An Lin &lt;cornelius.howl {at} gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DateTime::Format::DateParse" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="HTML::TokeParser::Simple" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Text::CSV_PP" />' >> $(DISTNAME).ppd
